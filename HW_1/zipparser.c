...
struct ZIP_File getZipFile(const char* file_name, long int startPosition) {
	...
	else
	{
		...
		if (lfh_position > 0)
		{
			...
			{
				printf("Нашли %dй LFH заголовок с помощью indexOf, (%ld)\n", fileIndex++, lfh_position);
		...
		printf("Нашли первый CDFH заголовок с помощью indexOf, (%ld)\n", cdfh_position);
		uint32_t eocd_signature = EOCD_Signature;
		long int eocd_position = indexOf(file, eocd_signature, startPosition);
		printf("Нашли первый EOCD заголовок с помощью indexOf, (%ld)\n", eocd_position);
